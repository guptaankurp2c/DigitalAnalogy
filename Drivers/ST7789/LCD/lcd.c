...
static void setSPI_Size(int8_t size){
  if(config.spi_sz!=size){
    ...
    if(size==mode_16bit){
      ...
      //LCD_HANDLE.Instance->CR1 |= SPI_CR1_DFF;  //This code is for STM32F103, changing to STM32F303
      LCD_HANDLE.Instance->CR2 |= (SPI_CR2_DS_0 | SPI_CR2_DS_1 | SPI_CR2_DS_2 | SPI_CR2_DS_3);
    }
    else{
      LCD_HANDLE.Init.DataSize = SPI_DATASIZE_8BIT;
      //LCD_HANDLE.Instance->CR1 &= ~(SPI_CR1_DFF);  //This code is for STM32F103, changing to STM32F303
      LCD_HANDLE.Instance->CR2 |= (SPI_CR2_DS_0 | SPI_CR2_DS_1 | SPI_CR2_DS_2 | SPI_CR2_DS_3); // first set to 1111
      LCD_HANDLE.Instance->CR2 |= ~(SPI_CR2_DS_3); //then make 0111
    }
  }
}
...
static void LCD_WriteCommand(uint8_t *cmd, uint8_t argc)
{
  ...
  LCD_PIN(LCD_DC_GPIO_Port,LCD_DC_Pin,RESET);
#ifdef LCD_CS
  LCD_PIN(GPIOC,LCD_CS_Pin,RESET);
#endif
  HAL_SPI_Transmit(&LCD_HANDLE, cmd, 1, HAL_MAX_DELAY);
  if(argc){
    LCD_PIN(GPIOC,LCD_DC_Pin,SET);
    HAL_SPI_Transmit(&LCD_HANDLE, (cmd+1), argc, HAL_MAX_DELAY);
  }
#ifdef LCD_CS
  LCD_PIN(GPIOC,LCD_CS_Pin,SET);
#endif
}
...
#endif
#ifdef LCD_RST
  LCD_PIN(GPIOC,LCD_RST_Pin,RESET);
  HAL_Delay(1);
  LCD_PIN(GPIOC,LCD_RST_Pin,SET);
  HAL_Delay(200);
#endif
  UG_Init(&gui, &device);
#ifndef LCD_LOCAL_FB
